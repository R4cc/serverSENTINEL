...
Minecraft runtime containers do not use Docker process-exit restart loops. serverSENTINEL owns lifecycle intent: deliberate stops remain stopped, deliberate restarts complete one stop/start cycle, and unexpected crashes retry after 5, 15, and 30 seconds. A fourth failure within 10 minutes activates crash-loop protection until an operator starts or stops the server. Servers that were running are also restored after the panel and node agents return from a host or Docker daemon restart.
...
- Schedules with delayed console commands and lifecycle actions such as Restart
- Settings
...
```text
nl2109/serversentinel:1.3.0
```

`nl2109/serversentinel:latest` tracks the newest stable image published from `main`. Use the pinned `1.3.0` tag when you want repeatable panel and node deployments.

The panel listens on port `8080` inside the container.

Recommended production setup:

- Use the pinned `nl2109/serversentinel:1.3.0` tag for panel and node agents.
...
services:
  serversentinel:
    image: nl2109/serversentinel:1.3.0
...
| `SERVERSENTINEL_NODE_IMAGE` | `nl2109/serversentinel:1.3.0` | backend | Image tag shown in generated node install/update instructions. |
...
Before every update, make a complete backup of the panel data root and managed server directory storage as described in [Backup And Restore](#backup-and-restore). For 1.3.0 this backup is mandatory: startup compacts schema 16 into the new schema-17 baseline and the compaction is not reversible in place.

serverSENTINEL 1.3.0 accepts only a fresh database or a database already fully migrated through schema 16. If the database is below schema 16, start it with serverSENTINEL 1.2.1 first, allow that release to finish its migrations, stop it cleanly, take a new backup, and only then start 1.3.0. Do not skip this staging upgrade.

Import/export artifacts are also intentionally compacted. Version 1.3.0 writes and accepts export schema 3 only; schema-1 and schema-2 artifacts must be imported into 1.2.1 and re-exported before upgrading.
...
3. Pull the target image, for example `docker pull nl2109/serversentinel:1.3.0`.
4. Update the panel container image tag and start the panel. Confirm it opens the database successfully before updating nodes.
5. In multi-node deployments, update node agents to the same tag shown by `SERVERSENTINEL_NODE_IMAGE`. This panel-first order lets 1.2.1 nodes reconnect during the short rolling-upgrade window.
...
Do not run 1.2.1 or an older image against a database already compacted to schema 17. Rollback requires restoring the complete pre-upgrade backup. SQLite migrations are forward-only.
...
```bash
docker build -t nl2109/serversentinel:1.3.0 -t nl2109/serversentinel:latest -f docker/Dockerfile .
```
