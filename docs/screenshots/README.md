...
The `README screenshots` GitHub Actions workflow uses a pinned Playwright container and opens or updates `automation/readme-screenshots` against `dev` when the generated PNGs change. The existing CI workflow is dispatched on that branch so required checks apply before the images merge into `dev` and later ship from `main`.
