  ...
  return (
    <section className="tabPage modsWorkspacePage">
      ...
      <div className="modsWorkspaceToolbar">
        ...
        <div className="modsWorkspaceUpdateActions"><Button variant="secondary" onClick={() => { if (!updateCheckWaitingForMods) void actions.refresh(); }} disabled={updateCheckWaitingForMods || state.updatePlanLoading} title={updateCheckWaitingForMods ? "Waiting for the current mod change to finish." : state.updatePlanLoading ? "Checking installed mods for updates." : "Check installed mods for updates."}><AppIcon name="refresh" /> {state.updatePlanLoading ? "Checking…" : "Check updates"}</Button>{showSafeBatch && <Button onClick={() => void actions.updateAllSafe()} disabled={!canRunSafeBatch}>{state.batchUpdateRunning ? "Updating safe mods…" : `Update all safe (${data.updatePlan?.counts.safeUpdates})`}</Button>}</div>
      </div>
