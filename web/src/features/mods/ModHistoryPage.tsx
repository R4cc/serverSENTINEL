...
import { api } from "../../api";
import { Button, PanelHeader, Surface, Toolbar } from "../../components/UiPrimitives";
...
export function ModHistoryPage({ serverId, terminology, locked, requestConfirmation, onBack, onChanged, formatDate, handleStaleSession, source }: {
  serverId: string;
  ...
  return <section className="tabPage modHistoryPage layoutWide">
    ...
    <div className="modHistoryHeading"><h2 ref={heading} tabIndex={-1}>{terminology.singularTitle} update history</h2><p>Installs, updates, removals and state changes. The latest 500 changes are retained with their saved jars.</p></div>
    ...
    <Surface className="modHistorySurface">
      <PanelHeader title="Recent changes" description="Revert restores the saved file, version and enabled state. Changes made before history was introduced are not included." />
      {loading ? <InlineState tone="loading" title="Loading history…" /> : !data?.entries.length ? <InlineState tone="empty" title="No changes recorded" message={`New ${terminology.singular} changes will appear here.`} /> : <>
        <div className="modHistoryTableScroll" role="region" aria-label={`${terminology.singularTitle} history table`} tabIndex={0}>
          <table className="modHistoryTable"><thead><tr><th scope="col">{terminology.singularTitle}</th><th scope="col">Action</th><th scope="col">Version change</th><th scope="col">Date</th><th scope="col">User</th><th scope="col">Revert</th></tr></thead>
            <tbody>{data.entries.map((entry) => <tr key={entry.id}>
              <td><strong>{entry.modName}</strong><small>{entry.after?.filename || entry.before?.filename}</small></td>
              <td data-label="Action"><div><span className="modHistoryAction">{entry.action}</span>{entry.revertsEntryId && <small>Revert</small>}</div></td>
              <td data-label="Version"><div><span>{entry.before ? entry.before.version || "Unknown version" : "Not installed"}</span><span className="modHistoryArrow" aria-label="to"> → </span><span>{entry.after ? entry.after.version || "Unknown version" : "Not installed"}</span>{(entry.before?.enabled === false || entry.after?.enabled === false) && <small>{entry.before ? entry.before.enabled ? "Enabled" : "Disabled" : "Absent"} → {entry.after ? entry.after.enabled ? "Enabled" : "Disabled" : "Absent"}</small>}</div></td>
